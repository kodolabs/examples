...
feature 'HostProviderAccount' do
  ...
  describe 'create new host provider account', js: true do
    it 'success' do
      visit new_host_provider_accounts_path
      ...
      click_button 'Create'
      expect(page).to have_flash I18n.t('notifications.host_provider_account_created')
    ...
    it 'create with new provider', js: true do
      ...
      expect(page).to have_flash I18n.t('notifications.host_provider_account_created')
      expect(Account.count).to eq 1
      expect(Account.first.provider.provider_type).to eq 'host'
    end
  end
end
