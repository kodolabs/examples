...
describe Dashboard::AddedClientsProvider do
  let!(:user) { create :user }

  let!(:client1) { create :client, name: 'James', created_at: Time.zone.now + 5.days }
  let!(:client2) { create :client, name: 'Corey', created_at: Time.zone.now + 3.days }
  ...
  describe 'Recently added clients' do
    it 'should return campaigns amount only with active clients' do
      clients = Dashboard::AddedClientsProvider.new.call
      sum1 = [
        campaigns_service1,
        campaigns_service2
      ].sum(&:monthly_spend)
    ...
    it 'should not include deactivate clients' do
